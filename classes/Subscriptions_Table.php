...
class Subscriptions_Table extends \WP_List_Table
{
    ...
    {
        $columns = array();
        $columns['created_at'] = __('Date/time', 'funding');
        $columns['customer_name'] = __('Name', 'funding');
        $columns['sub_amount'] = __('Amount', 'funding');
        $columns['sub_interval'] = __('Interval', 'funding');
        $columns['sub_status'] = __('Status', 'funding');

        $columns['subscription_id'] = __('Subscription ID', 'funding');
    ...
    function column_subscription_id($item)
    {
        ...
        $actions = array(
            'view'    => sprintf('<a href="%s">' . esc_html__('View', 'funding') . '</a>', $url_view),
            'cancel'    => sprintf('<a href="%s" style="color:#a00;" onclick="return confirm(\'' . __('Are you sure?', 'funding') . '\')">' . esc_html__('Cancel', 'funding') . '</a>', $url_cancel),
        );
    ...
    function getInterval($interval) {
        switch ($interval) {
            case '1 month':
                $return = esc_html__('Monthly', 'funding');
                break;
            case '3 months':
                $return = esc_html__('Each quarter', 'funding');
                break;
            case '12 months':
                $return = esc_html__('Annually', 'funding');
                break;
        }
    ...
    function getStatus($status) {
        switch ($status) {
            case 'pending':
                $return = esc_html__('Pending', 'funding');
                break;
            case 'active':
                $return = esc_html__('Active', 'funding');
                break;
            case 'canceled':
            case 'cancelled':
                $return = esc_html__('Cancelled', 'funding');
                break;
            case 'suspended':
                $return = esc_html__('Suspended', 'funding');
                break;
            case 'completed':
                $return = esc_html__('Completed', 'funding');
                break;
        }
