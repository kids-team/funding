...
const index = () => {
	...
	const showPaymentInfo = () => {
		...
		fetch(
			`/wp-admin/admin-ajax.php?action=funding_payment_info&project=${project}&country=${country}`
		)
	...
	return (
		<div className="grid grid--columns-2 grid--gap-12">
			<div>
				<form className="form grid grid--columns-1 grid--gap-8">
					...
					<div className="button-group button-group--right">
						<a
							...
							class={
								...
								(!amount || !country || !project
									? ' button--disabled'
									: '')
							}
							href="#/"
						>
							{__('Show Data', 'funding')}
						</a>
					</div>
			...
			<div className="result">
				{paymentInfo.result && (
					<div>
						<div className="card bg-white card--center card--no-image card--shadow">
							<h3 className="card__title">
								{__('Scan to pay', 'funding')}
							</h3>
							<SVG
								className="w-full"
								src={`/wp-admin/admin-ajax.php?action=funding_qr_code&project=${project}&country=${country}`}
							></SVG>
						</div>
